...
describe('소유 프로세스 실행', () => {
  ...
  it.skipIf(process.platform !== 'win32')('합성 환경값을 보존하고 helper 명령줄에 비밀을 노출하지 않는다.', async () => {
    ...
    const script = `const {createHash}=require('node:crypto');
      ...
      const ps = '$ErrorActionPreference="Stop"; [Console]::Error.WriteLine("checkmate-query:started"); $line=[Console]::In.ReadLine(); [Console]::Error.WriteLine("checkmate-query:read"); Import-Module ([IO.Path]::Combine($PSHOME,"Modules","Microsoft.PowerShell.Utility","Microsoft.PowerShell.Utility.psd1")); [Console]::Error.WriteLine("checkmate-query:utility"); $v=$line | ConvertFrom-Json; [Console]::Error.WriteLine("checkmate-query:input"); Import-Module ([IO.Path]::Combine($PSHOME,"Modules","CimCmdlets","CimCmdlets.psd1")); [Console]::Error.WriteLine("checkmate-query:cim"); $p=Get-CimInstance Win32_Process -Filter ("ProcessId = " + [int]$v.pid) -OperationTimeoutSec 30; [Console]::Error.WriteLine("checkmate-query:queried"); if ($null -eq $p -or $p.Name -ne "작업보호.exe") { "false"; exit 1 }; if ($p.CommandLine.Contains($v.secret) -or $p.CommandLine.Contains($v.base64)) { "false"; exit 1 }; "true"';
      ...
      process.stdout.write(JSON.stringify({hash:createHash('sha256').update(process.env.CHECKMATE_SECRET).digest('hex'),
        ...
        queryStages:proof.stderr?.match(/checkmate-query:(?:started|read|utility|input|cim|queried)/g) ?? []}));`;
    ...
    try {
      ...
      expect(JSON.parse(result.stdout)).toEqual({ hash: expected, kept: true,
        ...
        queryStages: ['checkmate-query:started', 'checkmate-query:read', 'checkmate-query:utility',
          'checkmate-query:input', 'checkmate-query:cim', 'checkmate-query:queried'] });
    } finally { delete process.env.CHECKMATE_SYNTHETIC_SECRET; }
  }, 75000);
